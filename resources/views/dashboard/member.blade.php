...
@section('content')
    <div class="row">
        ...
        @if(isset($member) && $member->is_parent == 1 && isset($children) && count($children) > 0)
        <div class="col-sm-12 mb-4">
            <div class="card">
                <div class="card-header">
                    <h5>{{ __('My Children') }}</h5>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @foreach($children as $child)
                                    <tr>
                                        ...
                                        <td>
                                            @if($child->membershipLates && $child->membershipLates->status == 'Pending')
                                                <button class="btn btn-sm btn-primary customModal buy_now" 
                                                    data-size="lg" 
                                                    data-id="{{ Crypt::encrypt($child->membershipLates->plan_id) }}"
                                                    data-url="{{ route('membership-payment.edit', $child->membershipLates->plan_id) }}?plan={{ urlencode($child->membershipLates->plans->plan_name ?? '') }}&child_id={{ $child->id }}"
                                                    data-title="{{ __('Make Payment') }}">
                                                    {{ __('Pay Now') }}
                                                </button>
                                            @elseif($child->membershipLates && ($child->membershipLates->status == 'Expired' || ($child->membershipLates->expiry_date && $child->membershipLates->expiry_date < now())))
                                                <button class="btn btn-sm btn-warning customModal buy_now" 
                                                    data-size="lg" 
                                                    data-id="{{ Crypt::encrypt($child->membershipLates->plan_id) }}"
                                                    data-url="{{ route('membership-payment.edit', $child->membershipLates->plan_id) }}?plan={{ urlencode($child->membershipLates->plans->plan_name ?? '') }}&child_id={{ $child->id }}"
                                            ...
                                            @else
                                                <a href="{{ route('member.show', Crypt::encrypt($child->id)) }}" class="btn btn-sm btn-info">
                                                    {{ __('View Details') }}
                                                </a>
                                            @endif
                                        </td>
